...
class NewPost extends Component {
  ...
  handlePost() {
    ...
    fetch('https://devlogger-intensive.herokuapp.com/post/new', { method: 'POST', mode: 'cors', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title, content, author: token.name, userId: token.id }) })
      .then(res => {
        if (res.status === 200) {
